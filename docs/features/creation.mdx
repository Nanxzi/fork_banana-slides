...
If the task says that the MinerU token is expired, invalid, or unauthorized, update it in **Settings → MinerU Configuration**, run the **MinerU PDF Parsing** service test, then create the project again. See [Configuration](/configuration#mineru-pdf-parsing).
