...
<p>
  ...
  <b>Go from idea to presentation in minutes—no tedious layout work, request changes naturally, and step into the era of true "Vibe PPT".</b>
</p>
<p>
...
Have you ever found yourself in this predicament: your presentation is due tomorrow, yet your slides remain blank; you have countless brilliant ideas, but your enthusiasm is drained by the tediousness of layout and design?

We long to quickly create presentations that are both professional and aesthetically pleasing. While traditional AI PPT generation apps generally meet the demand for "speed," they still suffer from the following issues:
...
These flaws make it difficult for traditional AI PPT generators to simultaneously satisfy our two main needs: "speed" and "beauty." Even those claiming to be "Vibe PPTs" fall far short of being truly "vibe" in my eyes.

However, the emergence of the nano banana 🍌 model has changed everything. I experimented with 🍌pro for PPT page generation and found the results to be exceptional in quality, aesthetics, and consistency. It can accurately render nearly all text from the prompts and faithfully follow the style of reference images. So, why not build a native "Vibe PPT" application based on 🍌pro?
...
| **R&D and Industrialization of Intelligent Production Equipment for Prepared Dishes** | **The Evolution of Money: A Journey from Shells to Paper Currency** |
...
Supports three starting modes: **Ideas**, **Outlines**, and **Page Descriptions**, catering to various creative workflows.
...
- **Natural Language Editing**: Supports modifying outlines or descriptions using conversational prompts (e.g., "Change page three to a case study"), with AI responding and adjusting in real-time.
...
- **Multi-format Support**: Upload PDF, Docx, MD, Txt, and other files; the system automatically parses content in the background.
...
- **Inpainting**: Perform conversational modifications on specific areas (e.g., "Change this chart to a pie chart").
...
- **Export images as high-fidelity PPT pages with clean backgrounds and fully editable images and text**
...
- Supports configuration of various expression styles, multiple languages, and multiple voice tones
...
| Export Formats | Supports PDF, (non-editable image) pptx | **Export to PDF, (image or editable) pptx, and explainer video** |
...
- [2026-04-25]: Asset Toolbox launched — Added three new modes (Full-image editing, Marquee editing (overlay/replace), and Smart Erase) on top of the original asset generation, providing a unified entry point for one-stop operation.
- [2026-04-25]: Supports OpenAI official OAuth login to bind accounts. Once bound, Codex can be used directly as a text/image generation provider without manually entering an API Key (based on the official OpenAI OAuth PKCE authorization flow, non-reverse engineered).
...
- [2026-04-23]: Added support for the gpt-image-2 model. Additionally, editable background export effects have been improved due to model capability upgrades (Select "Generative Acquisition" under Settings -> Export Options -> Background Acquisition).
...
- [2026-02-09]: New Features and Optimizations
  * New Features
    * Supports pasting and instantly recognizing images on the homepage, outlines, and description cards, providing an improved interactive experience.
  ...
  * Fixes and Experience Optimizations
    * Fixed export-related 500 errors, reference file association timing, outline/page data misalignment, task polling errors, infinite polling for description generation, image preview memory leaks, and handling of partial failures during batch deletion.
    * Optimized format example prompts, HTTP error message copy, Modal closing experience, cleaned up old project localStorage, and removed redundant prompts when creating projects for the first time.
    * Several other optimizations and fixes.
- [2026-01-04]: v0.4.0 released: Editable PPTX export fully upgraded:
  * Maximizes the restoration of font size, color, bolding, and other styles from text within images.
  * Added support for recognizing text content within tables.
  * More precise logic for restoring text size and position.
  * Optimized export workflow, significantly reducing text remnants on background images after export.
...
| 🔄 In Progress | Support exporting editable .pptx with multi-layering and precise background removal |
| 🔄 In Progress | Web search |
| 🔄 In Progress | Agent mode |
| ✅ Completed | TTS narration video export (Chinese/English/Japanese voices, subtitles, Ken Burns effect) |
...
1. Deploy and launch this application with one click via Rainyun (High bandwidth, ideal for HD image generation and downloading. New users get a 15-day free trial)
...
If you are using **Windows or macOS**, please [install **Docker Desktop**](https://docs.docker.com/desktop/setup/install/windows-install/) first and ensure that Docker is running (Windows users can check the system tray icon; macOS users can check the menu bar icon), then follow the same steps in the documentation.
...
> **Friendly Reminder: The Google nano banana pro model interface has high costs; please be mindful of the usage expenses.**
```env
...
# Select the Provider for Text and Image Generation
...
# API Keys for Various Providers (Only configure the providers you need to use)
...
Google Cloud Vertex AI allows calling Gemini models through GCP service accounts, with free credits available for new users. Configuration steps:
...
> [!TIP]
> If you encounter network issues, you can uncomment the mirror configuration in the `.env` file and rerun the startup command:
> ```env
...
Note: This method is not applicable if you have manually modified the code; you must revert the code to the pulled version first.
...
[![NuGet](https://img.shields.io/nuget/v/nacos-sdk-csharp.svg?style=flat-square)](https://www.nuget.org/packages/nacos-sdk-csharp/)
[![NuGet](https://img.shields.io/nuget/dt/nacos-sdk-csharp.svg?style=flat-square)](https://www.nuget.org/packages/nacos-sdk-csharp/)
[![License](https://img.shields.io/badge/license-Apache%202-4EB1BA.svg?style=flat-square)](https://github.com/nacos-group/nacos-sdk-csharp/blob/master/LICENSE)
[![Build Status](https://github.com/nacos-group/nacos-sdk-csharp/workflows/Build/badge.svg?branch=master)](https://github.com/nacos-group/nacos-sdk-csharp/actions)

C# implementation of the Nacos client SDK.
...
services.AddNacosV2Config(x =>
{
    ...
    x.EndPoint = "";
    x.Namespace = "ef3076bd-348f-4315-998f-410a6a3b7829";
    x.ConfigUseRpc = true;
});

var serviceProvider = services.BuildServiceProvider();
var configService = serviceProvider.GetService<INacosConfigService>();

var dataId = "test-dataid";
var group = "test-group";

// Get config
var content = await configService.GetConfig(dataId, group, 5000);

// Listen config
await configService.AddListener(dataId, group, new TestListener());

// Publish config
var isPublishOk = await configService.PublishConfig(dataId, group, "test-content");

// Remove config
var isRemoveOk = await configService.RemoveConfig(dataId, group);
...
var serviceProvider = services.BuildServiceProvider();
var namingService = serviceProvider.GetService<INacosNamingService>();

var serviceName = "test-service";

// Register instance
await namingService.RegisterInstance(serviceName, "127.0.0.1", 8080);

// Get instances
var instances = await namingService.GetAllInstances(serviceName);

// Select one healthy instance
var instance = await namingService.SelectOneHealthyInstance(serviceName);

// Deregister instance
await namingService.DeregisterInstance(serviceName, "127.0.0.1", 8080);

// Subscribe service
await namingService.Subscribe(serviceName, new TestEventListener());

public class TestEventListener : IEventListener
{
    public Task OnEvent(IEvent @event)
    {
        if (@event is NamingEvent namingEvent)
        {
            // Do something with the naming event
        }
...
`nacos-sdk-csharp` supports reading configuration from `appsettings.json`.

```json
{
  "nacos": {
    "ServerAddresses": [ "http://localhost:8848" ],
    "Namespace": "ef3076bd-348f-4315-998f-410a6a3b7829",
    "ConfigUseRpc": true,
    "NamingUseRpc": true
  }
}
...
```csharp
// Configuration Management
services.AddNacosV2Config(configuration.GetSection("nacos"));

// Service Discovery
services.AddNacosV2Naming(configuration.GetSection("nacos"));
```

### About ASP.NET Core Integration

For better integration with ASP.NET Core, such as using Nacos as a configuration source or for automatic service registration, please refer to the following repository:

[nacos-sdk-csharp-extensions](https://github.com/nacos-group/nacos-sdk-csharp-extensions)
...
- [nacos-sdk-java](https://github.com/alibaba/nacos/tree/master/client)
...
Follow the author on social media, where I will share updates about this project and AI-related content:
...
This project is open-sourced under the **GNU Affero General Public License v3.0 (AGPL-3.0)** and can be freely used for non-commercial purposes such as personal study, research, testing, education, or non-profit scientific research activities;
