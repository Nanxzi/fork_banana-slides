...
<p>
  ...
  <b>Go from idea to presentation in minutes—no tedious layout work, just prompt for modifications. Experience the true "Vibe PPT".</b>
</p>
<p>
...
- **[2026-06-23]**: Page-by-Page Templates Launched — Supports both single-template and multi-template modes. You can upload images or PDFs to build a project template library. The AI automatically analyzes template styles and intelligently matches them to each page with one click, or you can manually bind them page by page. Seamless two-way switching between the two modes is supported at any time ([Documentation](https://docs.bananaslides.online/zh/features/templates))
- **[2026-04-25]**: Asset Toolbox Launched — Adds three new modes on top of the original asset generation: full-image editing, marquee/box-select editing (overlay/replace), and smart erase, providing a unified entry for one-stop operations
- **[2026-04-25]**: Supports account binding via official OpenAI OAuth login. Once bound, Codex can be used directly as a text/image generation provider without manually entering an API Key. Plus accounts can generate 100+ 2K images within 5 hours ([Tutorial](https://ziy68cvfvu3.feishu.cn/wiki/LDSOwPzkhiNonkkNTF1ct2VBnNc)) (Based on the official OpenAI OAuth PKCE authorization flow, non-reverse engineered)
...
- **[2026-04-23]**: Added support for the gpt-image-2 model, while the editable background effect during export has also been enhanced due to model upgrades (Select "Generative Acquisition" under Settings -> Export Options -> Background Acquisition)
- **[2026-04-11]**: Added support for [CLI operations and introduced agent skills](https://docs.bananaslides.online/cli)
...
- **[2026-02-09]**: New Features and Optimizations
  * New Features
    * Supports pasting images on the homepage, outline, and description cards for instant recognition, providing a better interactive experience.
  ...
  * Fixes & Experience Optimizations
    * Fixed export-related 500 errors, reference file association timing, outline/page data misalignment, task polling errors for incorrect projects, infinite polling in description generation, image preview memory leaks, and partial failure handling during batch deletion.
    * Optimized format example prompts, HTTP error message copy, Modal closing experience, cleanup of localStorage for old projects, and removed redundant prompts for first-time project creation.
    * Various other optimizations and fixes

## ✨ Project Origin

Have you ever found yourself in this dilemma: you have a presentation tomorrow, but your PPT is still a complete blank; you have countless brilliant ideas in your head, but all your enthusiasm is worn away by tedious layout and design?

We aspire to quickly create presentations that are both professional and well-designed. Although traditional AI PPT generation apps generally meet the need for "speed", they still suffer from the following problems:

- 1️⃣ Can only choose preset templates, unable to flexibly adjust styles
- 2️⃣ Low freedom, making multi-round revisions difficult to execute
- 3️⃣ The finished products look similar, suffering from severe homogenization
- 4️⃣ Low material quality, lacking targeted relevance
- 5️⃣ Fragmented text-image layout, poor sense of design

These shortcomings make it difficult for traditional AI PPT generators to simultaneously satisfy our two major needs for PPT creation: "speed" and "beauty". Even if they claim to be "Vibe PPT", they are still far from enough "Vibe" in my eyes.

However, the emergence of the nano banana🍌 model has changed everything. I tried using 🍌pro to generate PPT pages and found that the results were excellent in terms of quality, aesthetics, and consistency. Moreover, it can accurately render almost all text requested by the prompt and follow the style of the reference image. So, why not build a native "Vibe PPT" application based on 🍌pro?

## 👨‍💻 Applicable Scenarios

1. **Beginners**: Quickly generate beautiful PPTs with zero barrier to entry, requiring no design experience and eliminating the hassle of template selection
...
5. **Workplace Professionals**: Swiftly visualize business proposals and product presentations, with rapid adaptation to multiple scenarios
...
- **Multi-format Support**: Upload PDF, Docx, MD, Txt, and other file types for automatic background content parsing.
...
- **Automatic Image Storage**: Images parsed from documents are automatically added to the project asset library once the reference file is linked, allowing for direct reuse later.
...
- **Local Editing**: Perform verbal-style modifications on areas you're dissatisfied with (e.g., "Change this chart to a pie chart").
...
- **Multi-format Support**: Export to standard **PPTX** or **PDF** files with one click.
- **Playback Settings**: Enable page transition animations before exporting to PPTX. Supports classic effects including Fade, Page Turn, Pan, Wipe, Split, Blinds, Checkerboard, Clock, and more, with the option to multi-select and apply them randomly.
- **Perfect Fit**: Default 16:9 aspect ratio; no layout adjustments needed, ready for direct presentation.
...
- Supports multiple expression styles, multiple languages, and various voice options
...
| Export Formats | Supports exporting to PDF, (non-editable image) pptx | **Export as PDF, (image or editable) pptx, explainer video** |
...
| 🔄 In Progress | Support for exporting editable .pptx with multi-layered, precise background removal |
...
1. One-click deploy and launch this application via Rainyun (High bandwidth, suitable for HD image generation and downloading. Free trials available for new users).
...
> **The LLM API in this project follows the AIHubMix platform format as the standard. It is recommended to use [AIHubMix (click here to access)](https://aihubmix.com/?aff=17EC) to obtain an API key to reduce migration costs.**<br>
> **Friendly tip: The API costs for the Google Nano Banana Pro model are relatively high; please be mindful of usage costs.**
```env
...
> ```env
> # Uncomment the following in the .env file to use domestic mirror sources (China)
...
Note: If you have manually modified the code, this method is not applicable; you must first revert the code to the version it was at when it was pulled.
...
- Node.js 16+ and npm
- [FFmpeg](https://ffmpeg.org/) - Required for explanation video export; must include support for `libass` / `ass` subtitle filters
- A valid Google Gemini API key
- (Optional) [LibreOffice](https://www.libreoffice.org/) - Required for converting PPTX files to PDF when using the "PPT Refurbishment" feature. **It is recommended to convert PPTX to PDF locally before uploading.** Reason: Server-side rendering with LibreOffice may cause layout displacement due to missing fonts (such as Microsoft YaHei, Calibri, etc.) and may not fully restore some special effects. Uploading PDF files does not require LibreOffice. Docker users who still need PPTX upload support within the container can run:
  ```bash
...
Please provide the Chinese Markdown content you would like me to translate. Based on the requirements you've provided, I am ready to translate it into a professional, clear, and readable English version while maintaining all your specified formatting and technical terms.

Once you provide the content, I will output **only** the translated English version.
...
The frontend will automatically connect via Vite proxy to the backend service specified by `BACKEND_PORT` (default `http://localhost:5011`). To modify it, please set `BACKEND_PORT` in the `.env` file at the project root.
...
Welcome to suggest new features or provide feedback. I will also ~~casually~~ answer your questions.
...
<p>Special thanks to AIHubMix for sponsoring this project</p>
</div>
...
Special thanks to AI Huobao for sponsoring this project
