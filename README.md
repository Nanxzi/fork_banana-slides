...
> **桌面版配置与导出提示**：桌面安装包没有项目根目录 `.env`，请直接在「设置」中保存 API 配置。模型的独立 Provider 会覆盖默认 API；若测试显示了意外的 Provider，可在模型配置区点击「全部跟随默认配置」。桌面端导出会弹出系统保存对话框，并在文件实际写入所选位置后才判定下载完成；若写入失败会显示目标路径和错误信息，也可在「导出任务」面板重新下载。
